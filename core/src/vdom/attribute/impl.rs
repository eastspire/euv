...
impl AttributeValue {
    ...
    ///
    /// This function replaces the inline `Signal::create(...)` boilerplate
    /// that was previously generated by the `html!` macro for every
    /// attribute value containing an `if` condition.
    ///
    /// The returned `Self::Signal` is a value carrier created fresh on each
    /// render of the enclosing dynamic node: the signals read inside
    /// `compute` during that render register the dynamic node as their
    /// dependent, so any source change re-renders the node and rebuilds
    /// this attribute with the new value. No registry listener is kept —
    /// the former `subscribe_attr` registration was unreachable dead weight
    /// (its key could never be marked dirty) and aliased the dynamic-node
    /// key space of the update registry.
    ///
    /// # Arguments
    ///
    ...
    ///   carrying the freshly computed attribute value.
    ...
    /// If any value is signal-backed, the result becomes a `Signal` attribute
    /// carrying the freshly merged value for this render; the enclosing
    /// dynamic node re-renders on any constituent signal change (see
    /// [`AttributeValue::reactive`]).
    ///
    /// # Arguments
    ///
